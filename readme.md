...
When SDL refuses to open a detected game controller, the input backend now logs the failure with the `SDL_GetError()` message and aborts the remainder of the hot-plug handling so no subsequent code dereferences a `NULL` handle.  The hot-unplug close path mirrors that guard so unexpected device enumeration blips write an informative diagnostic line instead of crashing inside the formatter.
