...
When the FBO uses a depth texture attachment, the allocation now mirrors the scaled framebuffer width and height so completeness checks succeed even if the overlay scale alters the aspect ratio.  This resolves the black frames produced by mismatched depth attachments and silences the related GL debug messages during startup.
