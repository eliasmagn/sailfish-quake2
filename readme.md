...
When a Sailfish SDK sysroot omits the `dbus-1.pc` metadata, the helper script now probes the sysroot for the canonical D-Bus include directories (`usr/include/dbus-1.0` alongside the companion `dbus-1.0/include` subdirectories) and feeds those paths directly to the Quake II makefiles.  The build only aborts after emitting a targeted message when neither pkg-config nor the expected headers are available, so environments that ship the headers without pkg-config files no longer fail mid-run.
